...
```{r Supplemental Figure 2 panel A, message=FALSE, warning=FALSE}
VlnPlot(infertile, "percent.mt", group.by = 'cell.type', pt.size = 0) + 
  ggtitle("% mitochondrial genes/cell") + NoLegend()
VlnPlot(infertile, "nFeature_RNA", group.by = 'cell.type', pt.size = 0, y.max = 3000) + 
  ggtitle("number of genes/cell") + NoLegend()
VlnPlot(infertile, "nCount_RNA", group.by = 'cell.type', pt.size = 0, y.max = 30000) + 
...
meta[meta$cell.type == 'vSMC', "cell.cat"] <- "vSMC"
meta[meta$cell.type == 'blood', "cell.cat"] <- "blood"
meta[meta$cell.type == 'macrophages 1', "cell.cat"] <- "macrophages"
meta[meta$cell.type == 'macrophages 2', "cell.cat"] <- "macrophages"
meta[meta$cell.type == 'macrophages 3', "cell.cat"] <- "macrophages"
meta[meta$cell.type == 'T cells 1', "cell.cat"] <- "T cells"
meta[meta$cell.type == 'T cells 2', "cell.cat"] <- "T cells"
meta[meta$cell.type == 'T cells 3', "cell.cat"] <- "T cells"
meta[meta$cell.type == 'T cells 4', "cell.cat"] <- "T cells"
meta[meta$cell.type == 'T cells 5', "cell.cat"] <- "T cells"
meta[meta$cell.type == 'Treg', "cell.cat"] <- "T cells"
meta[meta$cell.type == 'NK cells', "cell.cat"] <- "NK"
meta[meta$cell.type == 'B cells', "cell.cat"] <- "NK"
meta[meta$cell.type == 'neutrophils', "cell.cat"] <- "neutrophils"
meta[meta$cell.type == 'leukocytes', "cell.cat"] <- "leukocytes"
meta[meta$cell.type == 'unknown immune', "cell.cat"] <- "unknown immune"
cell.cats <- as.vector(meta$cell.cat)
infertile <- AddMetaData(infertile, cell.cats, 'cell.cat')

my_levels <- c("germ cells", "Sertoli & Leydig", "vSMC", "blood", "macrophages", "T cells", 
               "NK", "neutrophils", "leukocytes", "unknown immune")
infertile@meta.data$cell.cat <- factor(x = infertile@meta.data$cell.cat, levels = my_levels)

rm(meta, cell.cats)

VlnPlot(infertile, "percent.mt", group.by = 'cell.cat', pt.size = 0) + 
  ggtitle("% mitochondrial genes/cell") + NoLegend()
VlnPlot(infertile, "nFeature_RNA", group.by = 'cell.cat', pt.size = 0, y.max = 3000) + 
  ggtitle("number of genes/cell") + NoLegend()
VlnPlot(infertile, "nCount_RNA", group.by = 'cell.cat', pt.size = 0, y.max = 30000) + 
  ggtitle("number of transcripts/cell") + NoLegend()

```

```{r Supplemental Figure 2 panel B, message=FALSE, warning=FALSE}
genes <- c("ddx4", "sycp3", "ccnb3", "gsdf", "star", "tagln", "hbba1")
VlnPlot(infertile, genes, group.by = 'cell.cat', stack = T, flip = T) + NoLegend()
```
